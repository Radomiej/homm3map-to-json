...
import pl.radomiej.MapReader.Map.Version;
...
public class MapReader {
    ...
    private int readTown() throws IOException {
        ...
        if (map.version != Version.ROE) {
            stream.skip(9); //mustSpells?
        }

        stream.skip(9);//canSpells?
        ...
        for (int i = 0; i < castleEvents; i++) {
            ...
            stream.readInt(1); //players_affected

            if (map.version != Version.ROE) {
                stream.readInt(1); //human_affected
            }

            stream.skip(1); //ai_affected

            stream.skip(2); //day_of_first_event
            stream.skip(2); //event_iteration
                ...
                case RANDOM_DWELLING_FACTION:
                    ...
                    switch (object.obj) {
                        case RANDOM_DWELLING_LVL:
                            ...
                            break;
                        case RANDOM_DWELLING_FACTION:
                            object.def.spriteName = MapObject.getRandomDwellingDefNameByFactionAndLevel(new Random(System.currentTimeMillis()).nextInt(8), randomLevel);
                            break;
                        case RANDOM_DWELLING:
                            int randomCastleId = new Random(System.currentTimeMillis()).nextInt(8);
